function data = read_Intan_RHD2000(filename, simple)
...
% return values by Li Su 09-20-2018
...
data.sample_rate=frequency_parameters.amplifier_sample_rate;

if (data_present)
    data.t=t_amplifier';
    if (num_amplifier_channels > 0)
        data.analog_channels = amplifier_channels;
        data.analog_data = amplifier_data';
    end

    if (num_board_adc_channels > 0)
        data.digital_channels=board_adc_channels;
        data.digital_data = board_adc_data';
    end
    if (num_board_dig_in_channels > 0)
        data.dig_in_channels=board_dig_in_channels;
        data.dig_in_data=board_dig_in_data';
    end
end
...
assignin('caller', variable_name, variable);
